...
#         """Generate professional career recommendation report using Azure chat model with system prompt, excluding raw RAG data"""
#         try:
...
#             # System prompt for professional report generation
#             system_prompt = """
# You are a senior HR consultant specializing in talent development and career alignment for Fortune 1000 organizations. Your task is to generate a professional, actionable, and comprehensive assessment report based on the provided Genius Factor analysis data.

# Structure the report as follows:
# 1. **Executive Summary**: Provide a concise overview of identified Genius Factors, key insights, and high-level career recommendations. Highlight alignment with organizational goals.
# 2. **Genius Factor Profile**: Detail primary and secondary Genius Factors, including characteristics, strengths, and potential hybrid combinations.
# 3. **Current Alignment Analysis**: Evaluate talent-passion alignment, current role fit, and retention risks based on the data.
# 4. **Career Recommendations**: Offer specific role suggestions, industry mappings, and career pathways, incorporating cross-industry opportunities and hybrid factor synergies.
# 5. **Retention and Mobility Strategies**: Recommend organizational strategies for retention, internal mobility, and professional development tailored to the identified Genius Factors.
# 6. **Action Plan**: Outline a 30-90-180 day roadmap with specific goals, skill development opportunities, and recommended resources.
# 7. **Data Sources and Methodology**: Summarize the data sources used and the methodology, including the number of documents analyzed and the completeness of the data.

# Use formal language, incorporate bullet points and tables for clarity, and ensure the report is actionable, professional, and grounded in the provided data. Avoid including raw data dumps; synthesize the information into a polished narrative.
# """
...
#             # Prepare analysis_result for the prompt, excluding raw retrievedData
#             filtered_analysis = {
#                 "assessmentData": analysis_result.get("assessmentData", {}),
#                 "summary": analysis_result.get("summary", {}),
#                 "vectorStoreStats": analysis_result.get("vectorStoreStats", {})
#             }
#             data_str = json.dumps(filtered_analysis, indent=2)
...
#                 "metadata": {
#                     "processingTimestamp": "2025-08-19T00:29:00Z",
#                     "modelUsed": "Azure Chat Model (gpt-4o-mini)",
#                     "dataSourcesUsed": analysis_result.get('assessmentData', {}).get('sourcesCovered', []),
#                     "documentsAnalyzed": analysis_result.get('assessmentData', {}).get('totalDocumentsRetrieved', 0)
#                 }
#             }
...
#     @classmethod
#     def rebuild_vector_store(cls):
#         """Force rebuild of vector store"""
#         cls._vector_store = None
#         faiss_path = "faiss_index"
        
#         # Clean up existing files
#         for file_path in [faiss_path, f"{faiss_path}.hash"]:
...
#         return cls.initialize_vector_store(force_rebuild=True)
...
class AIService:
    ...
    @classmethod
    async def generate_career_recommendation(cls, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        """Generate professional career recommendation report using Azure chat model with system prompt"""
        try:
            if analysis_result.get('status') != 'success':
                return {
                    "status": "error",
            ...
            llm = AzureChatOpenAI(
                azure_endpoint=settings.AZURE_ENDPOINT,
                api_key=settings.AZURE_API_KEY,
                api_version=settings.AZURE_API_VERSION,  # e.g., "2023-05-15"
            ...
            # System prompt for professional report generation
            system_prompt = """
You are a professional HR consultant specializing in talent development and career alignment. Your task is to generate a comprehensive, professional assessment report based on the provided Genius Factor analysis data.

Structure the report as follows:
1. **Executive Summary**: Overview of identified Genius Factors, key insights, and high-level recommendations.
2. **Genius Factor Profile**: Detailed breakdown of primary and secondary factors, with characteristics and strengths.
3. **Current Alignment Analysis**: Assessment of talent-passion alignment and current role fit, including retention risk.
4. **Career Recommendations**: Specific role suggestions, industry mappings, and career pathways, including hybrid combinations.
5. **Retention and Mobility Strategies**: Organizational recommendations for retention, internal mobility, and development plans.
6. **Action Plan**: 30-90-180 day goals, skill development, and resources.
7. **Data Sources and Methodology**: Summary of retrieved data sources and assessment stats.

Use formal language, bullet points, tables where appropriate, and ensure the report is actionable and insightful. Base all content strictly on the provided data.
"""
